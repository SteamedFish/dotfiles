...
`pkgrel` 记录**同一上游版本**被重新打包的次数。它向用户发出信号：无需版本号升级、但需要重新构建。

### 规则一：未推送到 AUR 的更改不得更新 pkgrel

如果本地 PKGBUILD 的错误版本**从未推送到 AUR**，则没有任何用户安装过该版本。此时增加 `pkgrel` 会为从未分发的版本制造虚假的重建信号。

```
AUR 上有 1.7-1。你正在本地开发 1.8。
你发现本地 1.8 的 PKGBUILD 有错误并修复了它。
→ 不要增加 pkgrel。保持 pkgrel=1，继续迭代。
  没有人安装过你有问题的本地 1.8；没有什么可以递增的。
```

**规则：** 只有当你需要区分**已经在 AUR 上、用户可能已安装的版本**时，才递增 `pkgrel`。

### 规则二：仅在已安装用户需要重新构建时才更新 pkgrel

将某个版本推送到 AUR（`pkgver-1`）后，只有在安装了 `pkgver-1` 的用户**有需要重新构建的损坏安装**时，才将 `pkgrel` 升级至 `pkgver-2`。

| 更改类型 | 更新 pkgrel？ | 原因 |
|---------|-------------|------|
| 上游 Python 主要版本升级导致 site-packages 路径变化 | ✅ 是 | 用户安装的文件路径错误；需要重新构建 |
| 修复导致运行时崩溃的库链接问题 | ✅ 是 | 已安装的二进制文件损坏 |
| 修复 `install=` hook 主动损坏系统状态（写入错误文件、执行错误命令） | ✅ 是 | hook 主动破坏了已安装状态；需要清理并重建 |
| 补充遗漏的 `depends=()` 条目 | ❌ 否 | 成功安装的用户本就已有该依赖（否则构建会失败） |
| 修正 `makedepends=()` 条目 | ❌ 否 | 仅影响构建时；不影响已安装的包 |
| 更新 pkgdesc、url、license 元数据 | ❌ 否 | 仅外观改变；不影响已安装的文件 |
| PKGBUILD 风格/格式清理 | ❌ 否 | 不改变实际安装内容 |
| 补充遗漏的 `optdepends=()` 条目 | ❌ 否 | 可选依赖；不会破坏现有安装 |
| 修复仅打印错误信息的 `install=` hook | ❌ 否 | hook 仅为提示信息；已安装文件不受影响 |

**决策测试：** 问自己——"成功安装了 `pkgver-1` 的用户，在**不重新安装**的情况下，现在的安装是否已损坏或功能异常？" 若是 → 更新。若否 → 不更新。
